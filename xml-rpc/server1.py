...
class XMLRPC():
    def __init__(self, host="0.0.0.0", port=9000) -> None:
        ...
        self.server = SimpleThreadedXMLRPCServer(server_addr, requestHandler=RequestHandler)
        self.server.register_introspection_functions()
        self.server.register_function(self.callControllerServiceRequest)
        self.server.register_function(self.phoneGatewayEndCall)
        self.server.register_function(self.voicemailDetectResult)

    def callControllerServiceRequest(self, json_request: str):
        process = Process(target=self.startCall, args=[json_request])
        process.start()
        response = {
                "status" : 0,
                "msg" : "success"
    ...
    def phoneGatewayEndCall(self, json_response: str):
    ...
    def voicemailDetectResult(self, json_str: str):
        logger.debug("voicemailDetectResult message: {}".format(json_str))
        request = json.loads(json_str)
        if request and request['is_voicemail'] is True:
            server_response = self.sendToFreeswitchServer("http://%s:%s@%s:%s" % (pbx_username, pbx_password, pbx_host, pbx_port),
                            "uuid_setvar",
    ...
    def sendToFreeswitchServer(self, server_uri: str, functionName: str, content: str):
        try:
    ...
    def sendEndCallToCallControllerServer(self, server_uri: str, content: str):
        serv = xmlrpc.client.ServerProxy(server_uri)
        return serv.phoneGatewayEndCall(content)

    def startCall(self, json_request: str):
        ...
        if "-ERR" in server_response:
            ...
            logger.debug(self.sendEndCallToCallControllerServer(call_request.controller_url, call_response.__str__()))
