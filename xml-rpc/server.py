...
USER_NO_RESPONSE_CAUSE = ["USER_BUSY",
...
class CallRequest():
    def __init__(self, json_str: str) -> None:
        ...
        self.customer_number = "user/{0}".format(
            req_number) if '@' in req_number else "sofia/internal/{0}@{1}".format(req_number, pbx_public_ip)
        self.record_name = "{0}.wav".format(uuid4())
        self.local_record_path = "{0}{1}".format(
            record_folder, self.record_name)
        ...
        self.detect_vm = request[
            'detect_voicemail'] if "detect_voicemail" in request else detect_voicemail
        self.transfer = "TRANSFER_EXTENSION={0}".format(
            request["transfer_extension"]) if "transfer_extension" in request else ""
...
class CallResponse():
    def __init__(self, conversation_id: str, call_at: int, pickup_at: int, hangup_at: int, hangup_cause: str) -> None:
        ...
        self.status = 105 if hangup_cause == "CRASH" else (
            103 if hangup_cause in USER_NO_RESPONSE_CAUSE else 104)
...
def startCall(json_request: str):
    ...
    if "-ERR" in server_response:
        ...
        call_response = CallResponse(
            call_request.conversation_id, call_request.call_at, 0, time.time() * 1000, hangup_cause)
        logger.debug(call_response.__str__())
        logger.debug(sendEndCallToCallControllerServer(
            call_request.controller_url, call_response.__str__()))
...
def run_server(host="0.0.0.0", port=9000):
    ...
    with SimpleThreadedXMLRPCServer(server_addr, requestHandler=RequestHandler) as server:
        ...
        def phoneGatewayEndCall(json_response: str):
            logger.debug(
                "phoneGatewayEndCall message: {}".format(json_response))
            response = json.loads(json_response)
            print("audio_url: {}, time: {}".format(
                response["audio_url"], (response["hangup_at"] - response["pickup_at"])/1000))
            return json.dumps({
                "status": 0,
                "msg": "success"
            })
        ...
        def voicemailDetectResult(json_str: str):
            ...
            if request and request['is_voicemail'] is True:
                ...
                logger.debug("uuid_setvar {} response: {}".format(
                    request['conversion_id'], server_response))
                server_response = sendToFreeswitchServer("http://%s:%s@%s:%s" % (pbx_username, pbx_password, pbx_host, pbx_port),
                                                         "uuid_kill",
                                                         "{0}".format(request['conversion_id']))
                logger.debug("uuid_kill {} response: {}".format(
                    request['conversion_id'], server_response))
            return json.dumps({
                "status": 0,
                "msg": "success"
