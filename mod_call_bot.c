...
SWITCH_STANDARD_APP(call_bot_app_function)
{
	...
	while (switch_channel_ready(channel))
	{
		if (!isStarted)
		{
			if (switch_channel_pre_answer(channel) == SWITCH_STATUS_SUCCESS && switch_channel_test_flag(channel, CF_ANSWERED) && switch_channel_test_flag(peer_channel, CF_EARLY_MEDIA))
			{
