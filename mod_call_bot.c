...
static switch_status_t do_stop(switch_core_session_t *session, char *bugname)
{
	...
	if (bug)
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Received user command command, calling call_bot_session_cleanup\n");
		status = call_bot_session_cleanup(session, 0, bug);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "callbot stopped\n");
	}
...
static void event_process_response_handler(switch_event_t *event)
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Received event_process_response_handler with type: %s\n", actionType);
	// switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Received event_stop_audio_handler with session_id %s\n", sessionId);
	...
	else if (strcmp(actionType, ACTION_CALL_WAIT) == 0)
	{
		...
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "event_process_response_handler: hold call success!\n");
		}
		else
		{
	...
	else if (strcmp(actionType, ACTION_CALL_FORWARD) == 0)
	{
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "event_process_response_handler: transfer call with extension: %s, context: %s!\n", sip_extension, sip_domain);
		if (switch_ivr_session_transfer(session, sip_extension, NULL, sip_domain) == SWITCH_STATUS_SUCCESS)
		{
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "event_process_response_handler: transfer call success!\n");
		}
		else
		{
			switch_channel_clear_flag(channel, CF_HOLD);
...
static switch_bool_t capture_callback(switch_media_bug_t *bug, void *user_data, switch_abc_type_t type)
{
	...
	{
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Got SWITCH_ABC_TYPE_CLOSE, calling call_bot_session_cleanup.\n");
	...
	{
		// switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "Finished SWITCH_ABC_TYPE_READ.\n");
